...
<div id="display_Area">
   <div id="page_tabs_content">
	...
	$qury="insert into tbl_hrallowances (name,rate) values ('$name','$min')";
	...
	$func->addAuditTrail($activity,$username);
		echo "<script language=javascript>alert('Allowance Type Added Successfuly') </script>";
...
?>
	...
	$min=$_POST['minvu'];
	$date=date("Y-m-d H:i:s a");
	$qury="update tbl_hrallowances set rate='$min' where name='$name'";
	$resultq = mysql_query($qury);
	if(!$resultq){
	die('Invalid query: ' . mysql_error());
	}else{
	...
	$func->addAuditTrail($activity,$username);
		...
		echo "<script language=javascript>alert('Allowances Type Updated Successfuly') </script>";
		 echo "<script language=javascript>window.location='hr_allowances.php' </script>";
	}
	}
