	...
	$('#example').dataTable( {
		"footerCallback": function ( row, data, start, end, display ) {
			...
			$( api.column( 4).footer() ).html(
				''+numberWithCommas(Number(totalemployee).toFixed(2)) 
			);
			$( api.column( 5).footer() ).html(
				''+numberWithCommas(Number(totalemployer).toFixed(2)) 
			);
		$( api.column( 6).footer() ).html(
				''+numberWithCommas(Number(totalsum).toFixed(2)) 
			);
...
<div id="display_Area">
  <div id="page_tabs_content">
	...
	if( (isset($_REQUEST["date4"]) ? $_REQUEST["date4"] : "") || (isset($_REQUEST["date5"]) ? $_REQUEST["date5"] : "") ){
		...
		$query="select p.nssf, p.date_ref, p.payrollref,p.staff_ref, s.fname,s.mname,s.lname,s.idpass from tbl_hr_payslips as p inner join staff s on p.staff_ref=s.idpass where p.date_ref between '$from' and '$to' and p.nssf>0";
    ...
    <table class="borders" cellpadding="5" cellspacing="0">
      <tr style="height:30px;">
		...
		<?php 
		if( (isset($_REQUEST["date4"]) ? $_REQUEST["date4"] : "") || (isset($_REQUEST["date5"]) ? $_REQUEST["date5"] : "") ){ ?>
		<a href="pdf_hr_nssfreport.php?frm=<?php echo $from?>&to=<?php echo $to?>"  title="Click to Print" class="noline"><i class="icon icon-orange icon-print"></i>Print </a>
		<?php
		}else{ ?>
		<a href="pdf_hr_nssfreport.php?frm=all&to=all"  title="Click to Print" class="noline"><i class="icon icon-orange icon-print"></i>Print </a>
		<?php
		}
		?>		
		 </div>
			...
			?>
  			<tr>
			   <td width="5%"><?php echo $num?></td>
                ...
                <td align="right"><?php echo $row['nssf']?></td>
				 <td align="right"><?php echo $row['nssf']?></td>
				  <td align="right"><?php echo (($row['nssf'])*2)?></td>
              </tr>
			<?php
  			}
