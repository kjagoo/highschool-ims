...
function SubjectTally() {
 ...
 function getSubjectTallyMID($subjectgrade,$Value,$term,$year,$form,$classdef){
 $tally=array('tally' => '');
 $resultg = mysql_query("select count($subjectgrade) as engas from totalygradedmidterm where $subjectgrade='$Value' and term='$term' and year='$year' and form='$form' $classdef");
	while ($rowg= mysql_fetch_array($resultg)) {
		$count = $rowg['engas'];
		$tally['tally']  = $count;
	}
...
function getGradesPerSubjectEntireMID($subject,$subjectGrade, $term,$year,$form){
 
 $tally=array();
 $resultg = mysql_query("SELECT $subjectGrade AS grade,COUNT(`adm`)  AS total  FROM totalygradedmidterm WHERE  term=$term AND YEAR=$year AND form=$form and $subject > 0 GROUP BY $subjectGrade");
...
function getGradesPerSubjectMID($subject, $subjectGrade, $term,$year,$form,$stream){
 
 $tally=array();
 $resultg = mysql_query("SELECT $subjectGrade AS grade,COUNT(adm)  AS total  FROM totalygradedmidterm WHERE  term='$term' AND year='$year' AND form='$form' AND stream = '$stream'  and $subject > 0  GROUP BY $subjectGrade");
