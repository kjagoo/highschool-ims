...
/**
 * Class to interact with a mssql database
 */
class db_mssql {
    /**
     * Connection to mssql.
     ...
     * @var boolean
     */
    // ken kumbuka kutoa var $show_errors = false;
     ...
     * @param  string  Database user_name
    ...
    function db_mssql($db_host, $db_user, $db_pass, $db_name) {
        //$db_pass = $this->decrypt();
        $this->link = @mysql_connect($db_host, $db_user, $db_pass);
        
       // echo mssql_get_last_message() . " ". $db_host." ". $db_user." ". $db_pass;
    ...
    function prepare($value, $do_like = false) {
        ...
        return $this->mssql_real_escape_string($value, $this->link);
        /* ken
         * if (function_exists('mssql_real_escape_string'))
		{
			return mssql_real_escape_string($value, $this->link);
		}
		else
		{
			return mssql_escape_string($value);
		}
        */
    }
     ...
     */
    function error() {
        //	ken mysql error is mssql_get_last_message
        $this->error = (is_null($this->recent_link)) ? '' : mysql_error();
        //$this->error = "error";
        return $this->error;
    }
    ...
    function raise_error($error_message = '') {
        if ($this->recent_link) {
            ...
            // ken	$this->errno = $this->errno($this->recent_link);
        }
    ...
    /* ken .This function escapes the string to allow safe queries
         * It is a clone of mysql_real_escape_string which is not avilable for mysql
         *
    */
    function mssql_real_escape_string($data) {
        ...
        return $data;
    } // end fn mssql_real_escape_string

    function decrypt() {
        $filename = "e:/xampp/htdocs/eworkflow/db.inc";
        $handle = fopen($filename, "rb");
        $pass = fread($handle, filesize($filename));
        fclose($handle);
        $key = "eWorkflow";
