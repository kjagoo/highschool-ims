...
<div id="page_tabs">
  <ul>
    <li><a class="active" href="hr_deductions.php">Income Tax (P.A.Y.E.)</a></li>
...
<div id="display_Area">
   <div id="page_tabs_content">
    <fieldset>
     <table width="80%">
        <tr>
          <td valign="middle" rowspan="2"><a href="#openModal" class="btn btn-primary noline"><span class="icon icon-green icon-plus"></span> New PAYE Setting</a></td>
        </tr>
      </table>
    </fieldset>
    <div id="openModal" class="modalDialog">
        ...
        <form name="subjectform" action="<?php echo $_SERVER['PHP_SELF'] ?>" method="post">
          <table class="borders" style="margin-bottom: 5px;" cellpadding="5" cellspacing="0" width="100%">
            <tr style='height:30px;'>
              <td class='dataListHeader' colspan='4'><i class="icon icon-green icon-info"></i>&nbsp;Add Income Tax</td>
            </tr>
			<tr>
			...
			<tr>
              <td class="alterCell" width="20%"><b>Tax:</b></td>
              <td class="alterCell2"><input type="number" name="tax" class="inputFields"  required  tabindex="4"/></td>
            </tr>
            <tr>
	 ...
	 <?php
	if(isset($_POST['tax'])&& isset($_POST['minv'])&& isset($_POST['maxv'])){
	require_once("includes/dbconnector.php");
	...
	$qury="insert into tbl_hr_krapaye (minv,maxv,tax) values ('$min','$max','$tax')";
	$resultq = mysql_query($qury);
	if(!$resultq){
	die('Invalid query: ' . mysql_error());
	}else{
		echo "<script language=javascript>alert('PAYE Level Added Successfuly') </script>";
		 echo "<script language=javascript>window.location='hr_deductions.php' </script>";
	}
	}
	
?>
    <?php
    $result = mysql_query("SELECT * FROM tbl_hr_krapaye order by minv asc");
	
		?>
    <table class="borders" cellpadding="5" cellspacing="0">
      <tr style="height:30px;">
        <td class="dataListHeader">Default P.A.Y.E Settings</td>
      </tr>
      <tr>
