...
<div id="display_Area">
  ...
  <tr style='height:30px;'>
    <td class='dataListHeader' colspan='4'>Pocket Money
      <div style="float:right; margin-right:20px; width:60%;">
        <table width="80%" align="right">
          <tr>
            <td align="right"><a href="finance_pocketmoney_list.php" title="Refresh Page"><i class="icon icon-green icon-refresh"></i>Refresh</a></td>
          </tr>
        </table>
      </div></td>
  </tr>
  <td><table id="example" class="table table-striped table-bordered" cellspacing="0" width="100%">
        ...
        <?php
		$totals=0;
		  $result = mysql_query("select * from studentdetails where form NOT IN ('FORM 5','TRAN') order by admno desc");
		$num=0;
		  while($row = mysql_fetch_array($result)){ 
		  
		  if($fees->pocketMoneyBal($row['admno'])<=0){}else{
			  $num++;
		  ?>
        <tr class='record'>
		  <td><?php echo $num?> </td>
         ...
         <td><?php echo str_replace("_"," ",$row['fname'])." ".str_replace("_"," ",$row['lname'])." ".str_replace("_"," ",$row['sname'])?></td>
		  ...
		  <td align='right'><a href="pmstatement.php?ref=<?php echo $row['admno']?>">Statement</a></td>
        </tr>
        <?php 
		 $totals+=$fees->pocketMoneyBal($row['admno']);
		  	}
		  }
			?>
		</tbody>
