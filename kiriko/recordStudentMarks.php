...
$check=mysql_query("select * from examoutof  where years='$year' and subject='$subject'");
	...
	$examStatus=$rowstatus['s_status'];
	}
	if($examStatus=="Recorded"){
	
	
	if($cats==1){
	$marksq = "select tss.*,m.*, s.fname,s.lname,s.sname from tbl_studentsubjects as tss 
	inner join markscats m on tss.admno=m.admno
	inner join studentdetails as s on tss.admno=s.admno and tss.term=m.term and tss.year=m.year and tss.subject='$subject' and tss.form='$form' and m.form='$myform' and m.term='$term' and m.year='$year' and cat='1' and s.class='$strm'  order by s.admno ASC";
	...
	$marksq = "select tss.*,m.*, s.fname,s.lname,s.sname from tbl_studentsubjects as tss 
	inner join markscats m on tss.admno=m.admno
	inner join studentdetails as s on tss.admno=s.admno and tss.term=m.term and tss.year=m.year and tss.subject='$subject' and tss.form='$form' and m.form='$myform' and m.term='$term' and m.year='$year' and cat='2' and s.class='$strm'  order by s.admno ASC";
	...
	if($cats=='exams'){
		...
		$marksq = "select tss.*,m.*, s.fname,s.lname,s.sname from tbl_studentsubjects as tss 
	inner join marksemams m on tss.admno=m.admno
	inner join studentdetails as s on tss.admno=s.admno and tss.term=m.term and tss.year=m.year and tss.subject='$subject' and tss.form='$form' and m.form='$myform' and m.term='$term' and m.year='$year' and s.class='$strm'  order by s.admno ASC";
	
	//$marksq = "select m.*, s.fname,s.lname,s.sname from marksemams as m inner join studentdetails as s on m.admno=s.admno and m.form='$myform' and m.term='$term' and m.year='$year' and s.class='$strm' order by s.admno ASC";
...
$query = "SELECT tss.*,sud.* FROM tbl_studentsubjects as tss inner join studentdetails sud on tss.admno=sud.admno and tss.form='$form' and tss.stream='$strm' and tss.term='$term' and tss.year='$year' and tss.subject='$subject' order by tss.admno ASC";
