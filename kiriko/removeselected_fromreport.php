...
if(isset($_POST['delete'])){//CHECK IF THE DELETE BUTTON WAS CLICKED
	...
	$sql = "UPDATE tbleperformancetrackmock SET s_status='1' WHERE (admno='$del_id' and year='$yrtod' and term='$trmtod')";//CHANGE THE STATUS OF THE CONTACT TO SHOW THAT IT HAS BEEN DELETED
	$update_result = mysql_query($sql);
	$delete_result = mysql_query("delete from totalygradedmockmarks where adm='$del_id' and  year='$yrtod' and term='$trmtod'");
	...
	<script language=javascript>window.location="report_forms_view_mock.php?amode=<?php echo $mod?>&form=<?php echo $fm?>&term=<?php echo $trmtod?>&year=<?php echo $yrtod?>&stream=<?php echo $srm?>&gradeby=<?php echo $amode?>"</script>
