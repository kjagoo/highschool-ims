...
if($strm=='Entire'){
	...
	$Numbers = mysql_query("select count(adm) as total from totalygradedmidterm  where form='$form' and term='$term' and year='$year' $strmis");
	...
	$myqueryis="select * from totalygradedmidterm where  term='$term' and year='$year' and form='$form' $strmis order by averagepoints desc";
...
//get Form1  Mean
	...
	$getPositionClass = mysql_query("Select positionclass from positions where admno='$admno' and form='$form' $strmis and term='$term' and year='$year'");
	...
	/***********************************************************************************/
	$totalStudents=0;
	$getTotalstudents = mysql_query("select count(adm) as adms from totalygradedmidterm where term='$term' and year='$year' and form='$form'");
	while ($rowstuden = mysql_fetch_array($getTotalstudents)) {// get admno
	$totalStudents=$rowstuden['adms'];
	}
	$totalStudentsClass=0;
	$getTotalstudentsClass = mysql_query("select count(adm) as adms from totalygradedmidterm where term='$term' and year='$year' and form='$form' $strmis");
	...
	$getgrades="select * from totalygradedmidterm where term='$term' and year='$year' and adm='$admno'";
	...
	$engfinal=$rowg['eng1'];
	$enggrade=$rowg['eng1grade'];
	$kisfinal=$rowg['kis1'];
	$kisgrade=$rowg['kis1grade'];
	$mathfinal=$rowg['math1'];
	$mathgrade=$rowg['math1grade'];
	$biofinal=$rowg['bio1'];
	$biograde=$rowg['bio1grade'];
	$chemfinal=$rowg['chem1'];
	$chemgrade=$rowg['chem1grade'];
	$phyfinal=$rowg['phy1'];
	$phygrade=$rowg['phy1grade'];
	$hisfinal=$rowg['his1'];
	$hisgrade=$rowg['his1grade'];
	$geofinal=$rowg['geo1'];
	$geograde=$rowg['geo1grade'];
	$crefinal=$rowg['cre1'];
	$cregrade=$rowg['cre1grade'];
	$agrfinal=$rowg['agr1'];
	$agrgrade=$rowg['agr1grade'];
	$bstfinal=$rowg['bst1'];
	$bstgrade=$rowg['bst1grade'];
	$compfinal=$rowg['comp1'];
	$compgrade=$rowg['comp1grade'];
	$frefinal=$rowg['fre1'];
	$fregrade=$rowg['fre1grade'];
	$homefinal=$rowg['home1'];
	$homegrade=$rowg['home1grade'];
	$mscfinal=$rowg['music'];
	$mscgrade=$rowg['musicgrade'];
	$gradepoints=$rowg['fgrade'];
	$meangrade=$rowg['averagepoints'];
	$totalpoints=$rowg['totalpoints1'];
	
	$position=$rowg['position'];
	$positionclass=$rowg['positionclass'];
	$kcpepos=$rowg['kcpeposition'];
	...
	$freremarks=str_replace("&","'",$rowg['frenchremarks']);
	$homeremarks=$rowg['homeremarks'];
	$mscremarks=$rowg['musicremarks'];
	...
	$cat1totals=0;
	$cat1totals=$eng+$kis+$math+$bio+$chem+$phy+$his+$geo+$cre+$agr+$bst+$comp+$fre+$msc+$home;
	...
	$cat2totals=0;
	$cat2totals=$eng2+$kis2+$math2+$bio2+$chem2+$phy2+$his2+$geo2+$cre2+$agr2+$bst2+$comp2+$fre2+$msc2+$home2;
	...
	$agrt=$agr+$agr2+$agre;
	$bstt=$bst+$bst2+$bste;*/
	$tmarks=$engfinal+$kisfinal+$mathfinal+$biofinal+$chemfinal+$phyfinal+$hisfinal+$geofinal+$crefinal+$agrfinal+$bstfinal+$compfinal+$frefinal+$mscfinal+$homefinal;
	/*****************************************************************************************/
	if($form==1){
$myform='Form 1';
}
...
/*****************************************************************************************/
	$enginitials="-";
	$engint=mysql_query("select initials from initials where  form='$myform' $strmis and subject='English'");
	while ($roweng = mysql_fetch_array($engint)) {// get admno
	$enginitials=$roweng['initials'];
	}
	$kisinitials="-";
	$kisint=mysql_query("select initials from initials where  form='$myform' $strmis and subject='kiswahili'");
	while ($rowkis = mysql_fetch_array($kisint)) {// get admno
	$kisinitials=$rowkis['initials'];
	}
	$mathinitials="-";
	$mathint=mysql_query("select initials from initials where  form='$myform' $strmis and subject='math'");
	while ($rowmath = mysql_fetch_array($mathint)) {// get admno
	$mathinitials=$rowmath['initials'];
	}
	$bioinitials="-";
	$bioint=mysql_query("select initials from initials where  form='$myform' $strmis and subject='biology'");
	while ($rowbio = mysql_fetch_array($bioint)) {// get admno
	$bioinitials=$rowbio['initials'];
	}
	$cheminitials="-";
	$chemint=mysql_query("select initials from initials where  form='$myform' $strmis and subject='chemistry'");
	while ($rowchem = mysql_fetch_array($chemint)) {// get admno
	$cheminitials=$rowchem['initials'];
	}
	$phyinitials="-";
	$phyint=mysql_query("select initials from initials where  form='$myform' $strmis and subject='physics'");
	while ($rowphy = mysql_fetch_array($phyint)) {// get admno
	$phyinitials=$rowphy['initials'];
	}
	$hisinitials="-";
	$hisint=mysql_query("select initials from initials where  form='$myform' $strmis and subject='History'");
	while ($rowhis = mysql_fetch_array($hisint)) {// get admno
	$hisinitials=$rowhis['initials'];
	}
	
	$geoinitials="-";
	$geoint=mysql_query("select initials from initials where  form='$myform' $strmis and subject='geography'");
	while ($rowgeo = mysql_fetch_array($geoint)) {// get admno
	$geoinitials=$rowgeo['initials'];
	}
	$creinitials="-";
	$creint=mysql_query("select initials from initials where  form='$myform' $strmis and subject='cre'");
	while ($rowcre = mysql_fetch_array($creint)) {// get admno
	$creinitials=$rowcre['initials'];
	}
	$agrinitials="-";
	$agrint=mysql_query("select initials from initials where  form='$myform' $strmis and subject='agriculture'");
	while ($rowagr = mysql_fetch_array($agrint)) {// get admno
	$agrinitials=$rowagr['initials'];
	}
	$bstinitials="-";
	$bstint=mysql_query("select initials from initials where  form='$myform' $strmis and subject='bstudies'");
	while ($rowbst = mysql_fetch_array($bstint)) {// get admno
	$bstinitials=$rowbst['initials'];
	}
	$homeinitials="-";
	$homeint=mysql_query("select initials from initials where  form='$myform' $strmis and subject='Homescience'");
	while ($rowhome = mysql_fetch_array($homeint)) {// get admno
	$homeinitials=$rowhome['initials'];
	}
	
	$compinitials="-";
	$compint=mysql_query("select initials from initials where  form='$myform' $strmis and subject='Computer'");
	while ($rowcomp = mysql_fetch_array($compint)) {// get admno
	$compinitials=$rowcomp['initials'];
	}
	$freinitials="-";
	$freint=mysql_query("select initials from initials where  form='$myform' $strmis and subject='French'");
...
$pdf->SetFont('times', '', 10).
$pdf->Line(45, 42, 45, 122).//create next collumn
$pdf->Text(47, 46, 'CAT1').
$pdf->Text(47, 51, $eng).
$pdf->Text(47, 56, $kis).
$pdf->Text(47, 61, $math).
...
$pdf->Line(60, 42, 60, 122).//Next Column
$pdf->Text(62, 46, 'CAT2').
$pdf->Text(62, 51, $eng2).
$pdf->Text(62, 56, $kis2).
...
$pdf->Text(62, 116, $msc2).
$pdf->Text(62, 121, $bst2).

$pdf->Line(75, 42, 75, 122).
$pdf->Text(76, 46, 'EndTerm').
$pdf->Text(79, 51, "-").
$pdf->Text(79, 56, "-").
$pdf->Text(79, 61, "-").
$pdf->Text(79, 66, "-").
...
$pdf->Text(95, 116, $mscfinal).
$pdf->Text(95, 121, $bstfinal).


$pdf->Line(105, 42, 105, 122).

$pdf->Text(107, 46, 'Grade').
$pdf->Text(109, 51, $enggrade).
$pdf->Text(109, 56, $kisgrade).
$pdf->Text(109, 61, $mathgrade).
$pdf->Text(109, 66, $biograde).
$pdf->Text(109, 71, $phygrade).
$pdf->Text(109, 76, $chemgrade).
$pdf->Text(109, 81, $hisgrade).
$pdf->Text(109, 86, $geograde).
$pdf->Text(109, 91, $cregrade).
$pdf->Text(109, 96, $homegrade).
$pdf->Text(109, 101, $agrgrade).
$pdf->Text(109, 106, $compgrade).
$pdf->Text(109, 111, $fregrade).
$pdf->Text(109, 116, $mscgrade).
$pdf->Text(109, 121, $bstgrade).

$pdf->Line(120, 42, 120, 122).
...
$pdf->Text(125, 116, $mscPos).
$pdf->Text(125, 121, $bstPos).

$pdf->Line(135, 42, 135, 122).
*/
...
$pdf->Text(137, 116, $mscremarks).
$pdf->Text(137, 121, $bstremarks).

$pdf->Line(165, 42, 165, 122).
...
$pdf->Text(172, 116, "").
$pdf->Text(172, 121, $bstinitials).


$pdf->SetFont('times', 'b', 9).
$pdf->Text(17, 126, 'TOTALS MARKS').

$pdf->Text(47, 126, $cat1totals).
$pdf->Text(62, 126, $cat2totals).

$pdf->Text(79, 126, "-").
$pdf->Text(95, 126, $tmarks.'    Out of:  '  .$totalOutOf).

$pdf->SetFont('times', '', 9).
$pdf->Text(20, 130, 'Mean: 	   '.$meangrade.'   Total Points  '.$totalpoints.'        Grade:	 '.$gradepoints.'     Overall Position:	      '.$position .'       Out of:  '.$totalStudents).
$pdf->Text(140, 130, 'Position in Class:	     '.$positionclass.'       Out of:  '.$totalStudentsClass).
...
$pdf->SetFont('times', '', 9).
$pdf->Text(23, 215, 'Class Teacher Comments _____________________________________________________________________________________').
$pdf->Text(23, 220, '__________________________________________________________________________________________________________').
$pdf->Text(23, 225, '__________________________________________________________________________________________________________').
$pdf->Text(23, 230, '_______________________________________________________________ Signature __________________________________').
$pdf->Text(23, 240, "Principal's Comment ________________________________________________________________________________________").
$pdf->Text(23, 245, '__________________________________________________________________________________________________________').

//$pdf->Text(80, 244, $htremarks).
$pdf->Text(23, 250, '_______________________________________________________________ Signature __________________________________').
$pdf->Text(23, 260, 'Report Seen By Parent/Guardian Sign _______________________________ 	Date____________________________').
$pdf->Text(23, 265, 'Next TERM Begins On:   ').
$pdf->SetFont('times', 'BU', 9).
$pdf->Text(60, 264, $nexttermopens).
$pdf->SetFont('times', '', 7).
$pdf->Text(20,285,'Email:'.$email.', Website:'.$web.'',0,0,'C').' ','',1);
$pdf->Text(100,285,'Generated using Chrimoska LTD School Management System on '. $date,0,0,'R');	
}
}
$pdf->Output();
