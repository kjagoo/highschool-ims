...
if( (isset($_REQUEST["date4"]) ? $_REQUEST["date4"] : "") || (isset($_REQUEST["date5"]) ? $_REQUEST["date5"] : "") ){
 require_once("includes/dbconnector.php"); 
	...
	$sql="select distinct(receipt_no), admno,dateofpay,modeofpay,bank_account,total_amount,statusis from finance_feestructures where dateofpay between '$from' and '$to'  and statusis='OK' order by receipt_no asc";
...
<table class="borders" style="margin-bottom: 5px;" cellpadding="5" cellspacing="0" width="100%">
  <tr style='height:30px;'>
    <td class='dataListHeader' colspan='4'>Receipts Analysis for the Period: <?php echo $from." to - ".$to?>
      <div style="float:right; margin-right:20px">
        <table width="250px;">
          <tr>
		   <td align="center"><a href="finance_report_view_receipts.php?date4=<?php echo $from?>&date5=<?php echo $to?>"  class="noline" title="Refresh Page"><i class="icon icon-green icon-refresh"></i>Refresh</a></td>
			<td align="right"><a href="pdf_receipts.php?date4=<?php echo $from?>&date5=<?php echo $to?>" class="noline" title="Click to Print"><i class="icon icon-orange icon-print"></i>Print</a></td>
			<td align="right"><a href="csv_receipts.php?date4=<?php echo $from?>&date5=<?php echo $to?>" class="noline" title="Click to Export to CSV"><i class="icon icon-orange icon-xls"></i>Export CSV</a></td>
            <td align="right"><a href="finance_report_receipts.php"  class="noline" title="Previous Page" target="content"><i class="icon icon-red icon-close"></i>Back</a></td>
          </tr>
        </table>
      </div></td>
  </tr>
  <td><table id="example" class="table table-bordered table-striped">
       ...
       <?php
		...
		<tfoot>
        <tr>
          <th colspan="6" align="right" style="font-weight:bold; margin-right:20px;">Summary of Total Collection:</th>
