...
?>
 ...
 <div id="openModal" class="modalDialog">
      <div> <a href="#close" title="Close" class="close">X</a>
		<form name="subjectform1" action="save_additional_feesvotehead.php" method="post">
          <table class="borders" style="margin-bottom: 5px;" cellpadding="5" cellspacing="0" width="100%">
            <tr style='height:30px;'>
              <td class='dataListHeader' colspan='4'><i class="icon icon-green icon-info"></i>&nbsp;NEW VOTEHEAD FEES </td>
            </tr>
            
			 <tr>
              <td class="alterCell" width="25%"><b>Select Votehead:</b></td>
              <td class="alterCell2">
			  <select name="vh" class="select" required tabindex="2">
			  <?php
		  	$query=("select distinct(votehead) from finance_voteheads where fiscal_year='$finance' and term='$term'  and votehead not in (SELECT votehead FROM finance_fees where fiscal_yr='$finance' and term='$term' and form='$form')");
			...
			<tr>
              <td class="alterCell" width="25%"><b>AMOUNT:</b></td>
              <td class="alterCell2"><input type="text" size="35" name="amt" class="inputFields" tabindex="3" required/></td>
            </tr>
			
			
            <tr>
              <td class="alterCell" width="20%">&nbsp;</td>
              <td class="alterCell3"><input type="submit" name="submit" value="Save Fees" class="btn btn-primary"/></td>
            </tr>
          </table>
		  <input type="hidden" size="35" name="year" class="inputFields" tabindex="3" value="<?php echo $finance?>"/>
		  <input type="hidden" size="35" name="term" class="inputFields" tabindex="3" value="<?php echo $term?>"/>
		  <input type="hidden" size="35" name="form" class="inputFields" tabindex="3" value="<?php echo $form?>"/>
        </form>
  ...
  <tr style='height:30px;'>
    <td class='dataListHeader' colspan='4'>Available Set Fees for the Year: <?php echo $finance?>
      <div style="float:right; margin-right:20px">
        <table width="400px;">
          <tr>
		  <td><a href="#openModal" class=" noline"><i class="icon icon-red icon-plus"></i> Add Votehead</a></td>
		  
		  <td align="center"><a href="csv_feestructure.php?year=<?php echo $finance?>&form=<?php echo $form?>&term=<?php echo $term?>" title="Print xls"><i class="icon icon-green icon-xls"></i>Print XLS</a></td>
