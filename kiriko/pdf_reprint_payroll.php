...
$query="select p.*,s.category,s.staffno,s.kra_pin,s.idpass,s.fname,s.mname,s.lname from tbl_hr_payslips as p inner join staff s on p.staff_ref=s.idpass and p.payrollref='$pref'";
...
$ptr=0;
	$query_ref="select * from tbl_hr_payslips_reliefs  where payrollref='$pref' and relief_name='Personal Tax Relief'";
	$result_ref = mysql_query($query_ref);
	$rowsdf= mysql_num_rows($result_ref);
	if($rowsdf>1 || $rowsdf>0){
...
$pdf->Cell(80, 7,number_format(($paye+$ptr),2), 1, 0, "R", 1);
$pdf->Ln();
$pdf->setX(25);
...
$pdf->setX(25);
	...
	$pdf->Cell(80, 7,number_format($ptr,2), 1, 0, "R", 1);
	$pdf->Ln();
	$pdf->setX(25);
