...
$year=$_POST['yr'];
	...
	('$admn', '$year', '$term', '$votehead', '$amount') on duplicate key update amount='$amount'";//update cat 1
	 ...
	 $result = mysql_query($query);
		...
		?>
		<script language=javascript>window.location='finance_set_fees_additional_manage.php?admno=<?php echo $admn;?>&term=<?php echo $term;?>&yr=<?php echo $year;?>'</script>
		<?php
		}
...
echo "Form Error";
 ...
 echo "<script language=javascript>window.location='finance_additional_Fees_manage.php';</script>";
}
?>