...
if(isset($_POST['to']) && isset($_POST['yearf']) && isset($_POST['stream'])&& isset($_POST['message']) ){
...
foreach($smsconfig->getSMSDetails() as $row0){
	...
	if($stream=='all'){
	$query="select admno from studentdetails where year_finished='$yearf'";
	}else{
	$query="select admno from studentdetails where year_finished='$yearf' and class='$stream'";
	}
