...
function Header(){
	...
	$this->Text(75, 35, 'Cluster Exams Comparative Subjects Analysis Form '.$form.' Term '.$term.' Year '.$year);
...
if($positionby=="marks"){
	...
	$q="select distinct classin, (sum(averagepoints)/ count(adm)) as mean,  count(adm) as stds from totalygradedmockmarks where tgrade!='F' and form='$form' and term='$term' and year='$year' group by classin asc";	
	$mes=mysql_query($q);
	...
	$englishtallys = $stally -> getGradesPerSubjectMock("english","englishgrade", $term,$year,$form,$stream); 
