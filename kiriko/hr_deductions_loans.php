...
<div id="page_tabs">
  <ul>
   <li><a href="hr_deductions.php">Income Tax (P.A.Y.E.)</a></li>
	 ...
	 <li><a class="active" href="hr_deductions_loans.php">Other Deductions</a></li>
  </ul>
</div>
<div class="clear"></div>
<div id="display_Area">
  <div id="page_tabs_content">
    <fieldset>
     <table width="80%">
        <tr>
          <td valign="middle" rowspan="2"><a href="#openModal" class="btn btn-primary noline"><span class="icon icon-green icon-plus"></span>New Deduction</a></td>
        </tr>
      </table>
    </fieldset>
    <div id="openModal" class="modalDialog">
      <div> <a href="#close" title="Close" class="close">X</a>
        <form name="subjectform" action="<?php echo $_SERVER['PHP_SELF'] ?>" method="post">
          <table class="borders" style="margin-bottom: 5px;" cellpadding="5" cellspacing="0" width="100%">
            ...
            <tr>
              <td class="alterCell" width="25%"><b>Deduction Name:</b></td>
	 ...
	 <?php
	if(isset($_POST['name'])&& isset($_POST['minv']) && isset($_POST['staffno'])){
	require_once("includes/dbconnector.php");
	...
	$qury="insert into tbl_hr_loans (name,applies_to) values ('$name','$staffno')";
	...
	$func->addAuditTrail($activity,$username);
		echo "<script language=javascript>alert('Deduction has been Added Successfuly') </script>";
...
?>
    ...
    <table class="borders" cellpadding="5" cellspacing="0">
      <tr style="height:30px;">
        <td class="dataListHeader">Available Deductions</td>
      </tr>
      <tr>
        <td><table id="example" class="table table-striped table-bordered" cellspacing="0" width="100%">
			...
			?>
              <tr class="record">
				...
				<td align="right"><?php echo $row["applies_to"];?></td>
        ...
        <form name="subjectform" action="<?php echo $_SERVER['PHP_SELF'] ?>" method="post">
          <table class="borders" style="margin-bottom: 5px;" cellpadding="5" cellspacing="0" width="100%">
            ...
            <tr>
              ...
              <td class="alterCell2"><input  type="text" name="nameu" class="inputFields" autofocus required  tabindex="1" value="<?php echo str_replace("-"," ",$row["name"]);?>" style="background-color:#999999"/></td>
            </tr>
		  ...
		  <input  type="hidden" name="nameuo" class="inputFields"  required  tabindex="1" value="<?php echo $row["name"];?>" style="background-color:#999999"/>
        </form>
      </div>
    </div>
	...
	$name=$_POST['nameu'];
	$name=str_replace(" ","-",$name);
	$oldname=$_POST['nameuo'];
	//$min=$_POST['minvu'];
	$percentv=$_POST['percentv'];
	$date=date("Y-m-d H:i:s a");
	$qury="update tbl_hr_loans set name='$name' where  name='$oldname' and applies_to='$percentv'";
	$resultq = mysql_query($qury);
	if(!$resultq){
	die('Invalid query: ' . mysql_error());
	}else{
	$activity = "Updated HR other Deductions Settings ".$name;
	$func->addAuditTrail($activity,$username);

		echo "<script language=javascript>alert('Deduction has been  Updated Successfuly') </script>";
		 echo "<script language=javascript>window.location='hr_deductions_loans.php' </script>";
	}
	}
